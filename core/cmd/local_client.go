...
func (cli *Client) RunNode(c *clipkg.Context) error {
	updateConfig(cli.Config, c.Bool("debug"), c.Int64("ignore-before-height"))
...
func updateConfig(config *orm.Config, debug bool, ignoreBeforeHeight int64) {
	if debug {
		config.Set("LOG_LEVEL", zapcore.DebugLevel.String())
	}
	if ignoreBeforeHeight >= 0 {
		config.Set(orm.EnvVarName("IgnoreBeforeHeight"), ignoreBeforeHeight)
	}
}
