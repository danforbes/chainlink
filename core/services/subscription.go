...
func NewInitiatorSubscription(
	...
	if ignoreBeforeHeight := store.Config.IgnoreBeforeHeight(); ignoreBeforeHeight >= 0 {
		ignoreBeforeHeightBN := big.NewInt(ignoreBeforeHeight)
		if nextHead.Cmp(ignoreBeforeHeightBN) < 0 {
			nextHead = big.NewInt(0).Add(ignoreBeforeHeightBN, big.NewInt(1))
		}
	}
